"""
Example of following a Mastodon user using ActivityPub.
"""
...
from pyfed.models import APPerson, APFollow, APCreate
...
async def follow_user(resource="acct:kene29@mastodon.social"):
    ...
    try:
        ...
        # 4. Create Follow activity
        follow = APFollow(
            id=f"https://{CONFIG['domain']}/activities/{int(asyncio.get_event_loop().time() * 1000)}",
            actor=str(actor.id),
            object=actor_url,  # The person we want to follow
            published=datetime.utcnow().isoformat() + "Z",
            to=[actor_url]  # Send only to the target user
        )
        
        # 5. Deliver the Follow activity
        logger.info("Sending Follow activity...")
        result = await delivery.deliver_to_inbox(
            activity=follow.serialize(),
            inbox_url=inbox_url,
        ...
        if result.success:
            logger.info("Follow request sent successfully!")
