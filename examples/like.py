"""
Example of liking a Mastodon post using ActivityPub.
"""
...
from pyfed.models import APPerson, APLike
...
async def like_post(resource="acct:kene29@mastodon.social",
    ...
    try:
        ...
        like = APLike(
            ...
            actor=str(actor.id),
            object=post_url,  # The post we want to like
            published=datetime.utcnow().isoformat() + "Z",
            to=[actor_url]  # Send to the post author
        )
        
        # 5. Deliver the Like activity
        logger.info("Sending Like activity...")
        result = await delivery.deliver_to_inbox(
            activity=like.serialize(),
            inbox_url=inbox_url,
        ...
        if result.success:
            logger.info("Like activity sent successfully!")
            logger.info(f"Successfully delivered to: {result.success}")
        else:
            logger.error("Like activity failed!")
            if result.failed:
